...
const server = net.createServer((socket) => {
    ...
    socket.on('data', (data) => {
        ...
        console.log('Got some data from client:', socket.remoteAddress, socket.remotePort);
        if (!socketFullyOpen(socket)) {
            console.log('socket not readable or writable for:', socket.remoteAddress, socket.remotePort);
            return;
        }
            ...
            switch (status.type) {
                case FRAME_TYPE.POP:
                    cancelPopRequest = dataStack.requestPop( (payload) =>{
                        console.log('Popping data off for client:', socket.remoteAddress, socket.remotePort);
                ...
                case FRAME_TYPE.PUSH:
                    cancelPushRequest = dataStack.requestPush(payload, () => {
                        console.log('Pushing data onto stack for client:', socket.remoteAddress, socket.remotePort);
        ...
        socket.on('end', () => {
            ...
            if (typeof cancelPopRequest === 'function') {
                cancelPopRequest();
                console.log('Cancelling pending pop request for:', socket.remoteAddress, socket.remotePort);
            }
            if (typeof cancelPushRequest === 'function') {
                cancelPushRequest();
                console.log('Cancelling pending push request for:', socket.remoteAddress, socket.remotePort);
            }
        });
